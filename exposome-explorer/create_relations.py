    ...
    return tx.run("""
        LOAD CSV WITH HEADERS FROM 'file:///cancer_associations.csv' AS line
            MATCH
                ...
                (c:Disease { Exposome_Explorer_ID: "Cancer_"+line.`cancer_id` })
    ...
    return tx.run("""
        LOAD CSV WITH HEADERS FROM 'file:///measurements.csv' AS line
            MATCH
                ...
                (c:Metabolite { Exposome_Explorer_ID: "Component_"+line.component_id }),
    ...
    return tx.run("""
        LOAD CSV WITH HEADERS FROM 'file:///samples.csv' AS line
            MATCH
                ...
                (sp:BioSpecimen { Exposome_Explorer_ID: "Specimen_"+line.specimen_id }),
...
def microbial_metabolite_identifications(tx):
    ...
    return tx.run("""
        LOAD CSV WITH HEADERS FROM 'file:///microbial_metabolite_identifications.csv' AS line
            MATCH
                (m:Metabolite { Exposome_Explorer_ID: "MicrobialMetabolite_"+line.id, Microbial_Metabolite:"True" }),
                (c:Metabolite { Exposome_Explorer_ID: "Component_"+line.component_id }),
                (p:Publication { Exposome_Explorer_ID: "Publication_"+line.publication_id }),
                (s:BioSpecimen { Exposome_Explorer_ID: "Specimen_"+line.specimen_id })
